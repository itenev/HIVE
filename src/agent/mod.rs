...
impl AgentManager {
    pub fn new(provider: Arc<dyn Provider>, memory: Arc<MemoryStore>) -> Self {
        ...
        let search_timeline = ToolTemplate { name: "search_timeline".into(), system_prompt: "Deep search the infinite long-term episodic memory logs. 'action:[search] query:[text] limit:[N]' — search current channel. 'action:[search] query:[text] scope:[channel_id]' — search a specific other channel. 'action:[search] query:[text] scope:[all_public]' — search ALL public channels.".into(), tools: vec![] };
